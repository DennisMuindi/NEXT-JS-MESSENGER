import { useState } from "react";
import Log_out from "../auth/logout";

export default function ArrowDownUp() {
  const [isOpen, setIsOpen] = useState(false);

  function handleClick() {
    setIsOpen(!isOpen);
  }

  return (
    <div className="relative">
      <button
        className="flex items-center justify-center rounded-md px-4 py-0 transition-colors duration-300 ease-in-out"
        onClick={handleClick}
      >
        <svg
          width="18"
          height="18"
          viewBox="0 0 18 18"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M3.35225 5.97725C3.55195 5.77755 3.86445 5.7594 4.08466 5.92279L4.14775 5.97725L9 10.8293L13.8523 5.97725C14.052 5.77755 14.3645 5.7594 14.5847 5.92279L14.6477 5.97725C14.8474 6.17695 14.8656 6.48945 14.7022 6.70966L14.6477 6.77275L9.39775 12.0227C9.19805 12.2224 8.88555 12.2406 8.66534 12.0772L8.60225 12.0227L3.35225 6.77275C3.13258 6.55308 3.13258 6.19692 3.35225 5.97725Z"
            fill="black"
          />
        </svg>
        {/* <span className="font-medium">{isOpen ? "Hide" : "Show"} more</span> */}
      </button>
      <div
        className={`absolute top-full right-0 bg-white shadow-md rounded-md overflow-hidden transition-all duration-500 ease-in-out ${
          isOpen
            ? "opacity-100 scale-100 mt-2 "
            : "opacity-0 scale-0 -translate-y-full"
        }`}
      >
        <Log_out />
      </div>
    </div>
  );
}
